...
export default function F180AddToCollection({ verseId }: { verseId: string }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div
          className="absolute z-20 mt-2 w-80 rounded-xl border shadow-lg p-3"
          style={{
        ...
        >
          ...
          {err && <div className="text-sm mt-2" style={{ color: 'hsla(0,75%,60%,1)' }}>{err}</div>}
          ...
          {!loading && !err && collections.length > 0 && (
            <ul className="mt-2 divide-y" style={{ borderColor: 'hsl(var(--border))' }}>
              {collections.map((c) => (
